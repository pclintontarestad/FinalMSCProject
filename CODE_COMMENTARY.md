...
- `plot_su3_expectations` constructs the dark-adapted SU(3) basis from the
  isometric encoder via SVD (ensuring the bright axis is the orthogonal
  complement), classifies generators into logical vs leakage channels, lifts
  them to the full Hilbert space, and plots expectation values plus a normalized
  leakage radius `r_leak(t)`. 【F:tripod_sim.py†L497-L616】
