...
def mixing_angle_derivative_analytic(t: float) -> float:
    """
    Analytic dθ/dt = (Ωc·dΩp/dt - Ωp·dΩc/dt) / (Ωc² + Ωp²).

    Uses the pulsed σ± couplings as the probe amplitude and the control
    envelope for Ωc.
    """
    ...
    dt = 1e-3
    Oc_plus = np.abs(Omega_c(t + dt))
    Oc_minus = np.abs(Omega_c(t - dt))
    dOc_dt = (Oc_plus - Oc_minus) / (2 * dt)

    g_plus_plus = np.abs(g_plus_t(t + dt))
    g_plus_minus = np.abs(g_plus_t(t - dt))
    g_minus_plus = np.abs(g_minus_t(t + dt))
    g_minus_minus = np.abs(g_minus_t(t - dt))

    Op_plus = np.sqrt(g_plus_plus**2 + g_minus_plus**2)
    Op_minus = np.sqrt(g_plus_minus**2 + g_minus_minus**2)
    dOp_dt = (Op_plus - Op_minus) / (2 * dt)

    return (Oc * dOp_dt - Op * dOc_dt) / denom
