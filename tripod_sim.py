...
def H_tripod_t(t: float, args: dict) -> Qobj:
    ...
    # σ+ photon couples g_-1 ↔ e (Δm = +1)
    gplus_t = evaluate_coupling(g_plus_runtime, t)
    H += gplus_t * (a_plus * sig_egm1 + a_plus.dag() * sig_egm1.dag())

    # σ− photon couples g_+1 ↔ e (Δm = -1)
    gminus_t = evaluate_coupling(g_minus_runtime, t)
    H += gminus_t * (a_minus * sig_egp1 + a_minus.dag() * sig_egp1.dag())
