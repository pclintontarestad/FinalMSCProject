...
#
# Dimensional constants (Hz, seconds) are defined first, then converted to the
# dimensionless simulation frame where all frequencies are scaled by Γ_e and
# times by Γ_e⁻¹. ``apply_sweep_parameters`` recomputes the derived globals so
# sweep runs refresh the time grids and couplings before any evolution calls.
# ------------------------------------------------------------------
...
class SweepParameters:
    ...
    control_center_phys: float = 2.0e-6
    probe_center_phys: float = 3.0e-6
...
#
# Defines the control/probe envelopes and the derived probe-scaled couplings
# ``g_±(t)``. All functions remain dimensionless. The mixing-angle helpers and
# diagnostics use these to check adiabaticity; retrieval reuses mirrored control
# shapes while the probe envelope stays diagnostic-only.
# ------------------------------------------------------------------
...
def mixing_angle(t: float) -> float:
    ...
    Oc = Omega_c(t)
    g_amp = np.sqrt(np.abs(g_plus_t(t)) ** 2 + np.abs(g_minus_t(t)) ** 2)
    Op = g_amp
...
#
# Fixes the basis ordering (|e>,|g_-1>,|g_0>,|g_+1>) and lifts all atomic
# projectors and photon mode operators (σ+, σ−) into the joint tensor space so
# later routines can assemble Hamiltonians and measurements without recomputing
# identities.
# ------------------------------------------------------------------
...
#
# Builds H(t)/ħ with excited-state detuning, optional ground detunings, photon-
# assisted couplings driven by the probe envelope (g_±(t)), and the classical
# control Ω_c(t). Collapse-operator helpers can inject decay/dephasing when
# desired.
# ------------------------------------------------------------------
...
#
# Defines the canonical single-photon σ± Fock states and tensors them with the
# atomic reference |g₀⟩. These serve as entry points for encoder construction and
...
#
# Evolves σ± inputs through the write window, extracts ground-manifold
# amplitudes conditioned on photon vacuum, and assembles the 3×2 encoder
# matrices (E_phys, E_iso). Diagnostics report efficiencies, Gram eigenvalues,
# and conditioning so asymmetric cases can be spotted immediately.
# ------------------------------------------------------------------
...
#
# Single-window diagnostics for storage: atomic populations, photon numbers,
# bright-projector population, and dark-coupling residuals. Useful for quick
# checks of adiabaticity before running full memory cycles.
# ------------------------------------------------------------------
...
#
# Orchestrates the full memory protocol (write → settle → gate → read) with
# optional collapse operators. Retrieval fidelity, tomography, and photon return
# metrics live here so changes to gate/read Hamiltonians remain isolated.
# ------------------------------------------------------------------
...
def run_read_phase(psi_in: Qobj, c_ops=None, h_args: dict | None = None):
    ...
    def g_plus_read(t: float) -> float:
        mirrored_t = T_READ_END - t
        return -g_pulsed(mirrored_t, g_plus)

    def g_minus_read(t: float) -> float:
        mirrored_t = T_READ_END - t
        return -g_pulsed(mirrored_t, g_minus)

    args = {
        "delta_m": delta_m,
        "Omega_c_func": Omega_c_read,
        "g_plus": g_plus_read,
        "g_minus": g_minus_read,
    }
    if h_args:
        args.update(h_args)
...
#
# Bare Gell-Mann generators and dark/bright basis builders for the ground
# manifold. These feed the adapted-basis leakage metrics used to separate true
# bright population, dark–bright coherence, and encoder non-isometry.
# ------------------------------------------------------------------
...
def dark_bright_unitary_from_couplings(
    ...
    """Return the {|D₀⟩, |D₁⟩, |B⟩} basis aligned to instantaneous couplings.

    Uses the coupling row ``c = [g_- , Omega_c , g_+]`` (in bare |g₋₁>,|g₀>,|g₊₁>
    ordering), performs an SVD, and treats the first right singular vector as the
    bright axis while the remaining two span the dark nullspace. Columns are
    ordered ``[|D0>, |D1>, |B>]`` to match SU(3) logical/leakage classification.
    """
...
def logical_basis(
    ...
    Aligns |B> with the instantaneous coupling vector, then Gram–Schmidts a
    bare basis seed to define |0_L> orthogonal to |B>. The cross product yields
    |1_L>, giving an orthonormal logical qubit plus leakage direction for MS
    projections.
    """
...
def transform_and_classify_su3(U_atom: np.ndarray):
    """Rotate bare Gell-Mann generators into a supplied dark/bright basis.

    Uses ``U_atom`` (columns = |D0>,|D1>,|B>) to obtain λᵢ′ = U† λᵢ U. Each λᵢ′
    is block-decomposed: logical action lives in the upper 2×2 block, leakage in
    the off-block coupling to |B>. Returns rotated generators plus index sets for
    logical vs leakage channels.
    """
...
def su3_leakage_diagnostics(
    ...
    """Compute adapted-basis SU(3) metrics with a time-dependent bright axis.

    Rebuilds the dark/bright basis at every time from the instantaneous coupling
    vector ``[g_-, Omega_c(t), g_+]`` so bright population is physical, not a
    static encoder complement. Returns generator expectations, bright population,
    non-adiabatic dark–bright coherence, encoder non-isometry (||G-ηI||), and
    classification indices for logical vs leakage channels.
    """
    ...
    t_class = tlist[int(np.argmax(coupling_norms))]

    # Classify generators using the peak-coupling basis so labels are stable; expectations
    # are still evaluated with the fully time-dependent basis per step.
    U_init = dark_bright_unitary_from_couplings(
        omega_c_func(t_class),
        evaluate_coupling(g_minus_use, t_class),
        evaluate_coupling(g_plus_use, t_class),
    )
    ...
    # Encoder non-isometry along the write window from σ± trajectories
    res_plus = run_trajectory(1.0, 0.0)
    res_minus = run_trajectory(0.0, 1.0)
    coeffs_plus = extract_ground_trajectory(res_plus.states)
    coeffs_minus = extract_ground_trajectory(res_minus.states)
    ...
    nonadiabatic_coh = np.zeros(n_steps)
    logical_purity = np.zeros(n_steps)
    iso_gap = np.zeros(n_steps)
    ...
    for idx, (t, state) in enumerate(zip(tlist, res.states)):
        ...
        # Encoder non-isometry: ||G(t) - eta(t) I||_2 with G = E^†E from σ± images
        cp = coeffs_plus[idx]
        cm = coeffs_minus[idx]
        E_t = np.column_stack((cp, cm))
        G_t = E_t.conj().T @ E_t
        eta_t = 0.5 * np.trace(G_t).real
        iso_gap[idx] = np.linalg.norm(G_t - eta_t * np.eye(2), 2)
        ...
        if rho_g is not None and U_ms is not None:
            ...
            nonadiabatic_coh[idx] = np.sqrt(np.abs(rho_ms[0, 2]) ** 2 + np.abs(rho_ms[1, 2]) ** 2)
    ...
    return {
        ...
        "nonadiabatic_coherence": nonadiabatic_coh,
...
def plot_su3_leakage_dashboard(
    ...
    """Plot bright leakage, dark–bright coherence, and encoder non-isometry over time.

    A concise dashboard for the write window: top panel shows physical bright
    population, middle panel shows non-adiabatic dark–bright coherence, and the
    bottom panel shows the instantaneous encoder non-isometry metric
    ``||G(t)-η(t)I||``. Intended as the thesis-facing replacement for raw λᵢ plots.
    """
    ...
    nonadiabatic_coh = diag["nonadiabatic_coherence"]
    iso_gap = diag["iso_gap"]
    ...
    ax2.plot(t_us, nonadiabatic_coh, "--", label="Dark–bright coherence", lw=1.5)
    ax2.set_ylabel("Non-adiabaticity")
    ax2.legend()
    ax2.grid(True, alpha=0.3)

    ax3 = plt.subplot(3, 1, 3, sharex=ax1)
    ax3.plot(t_us, iso_gap, lw=2)
    ax3.set_ylabel(r"Encoder non-isometry $||G-\eta I||$")
    ...
    if collector is not None:
        collector.append(
            ...
            f"nonadiabatic_max={np.max(nonadiabatic_coh):.3e}, "
            f"iso_gap_max={np.max(iso_gap):.3e}"
        )
...
def plot_su3_expectations(
    ...
    if collector is not None:
        ...
        collector.append(
            "MS-basis metrics: "
            f"bright_max={np.max(diag['bright_pop']):.3e}, "
            f"nonadiabatic_max={np.max(diag['nonadiabatic_coherence']):.3e}, "
            f"iso_gap_max={np.max(diag['iso_gap']):.3e}"
        )
...
#
# Standalone validation sweep: integrates excited-state population over a range
# of two-photon detunings with the control on to reproduce the EIT transparency
# dip. Also reused by ``run_EIT_validation`` for a quick control on/off check.
# ------------------------------------------------------------------
...
if __name__ == "__main__":
    try:
        ...
        su3_diag = plot_su3_leakage_dashboard(
            E_iso, alpha_plus=1.0, alpha_minus=0.0, collector=run_outputs
        )

        run_EIT_validation()

        fidelity_sigma_plus, _ = compute_full_cycle_fidelity(
            alpha_plus=1.0, alpha_minus=0.0
        )
        print(
            f"\nFull write → gate → read fidelity for σ+: {fidelity_sigma_plus:.6f}"
        )
        run_outputs.append(
            f"Full-cycle fidelity for sigma+: {fidelity_sigma_plus:.6f}"
        )
