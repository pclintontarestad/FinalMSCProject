...
def report_retrieval_tomography() -> None:
    """Print retrieval fidelities for a small tomography set of inputs."""
    ...
    for label, (ap, am) in test_states:
        ...
        print(f"Retrieval fidelity for {label:6s}: {fidelity:.6f}")
...
def plot_su3_expectations(E_iso: np.ndarray, alpha_plus: complex = 1.0, alpha_minus: complex = 0.0):
    """
    ...
    for idx in leak_idxs:
        leakage_sq_sum += np.abs(exps[idx]) ** 2  # sum-of-squares across leakage channels
...
if __name__ == "__main__":
    try:
        ...
        plot_su3_expectations(E_iso, alpha_plus=1.0, alpha_minus=0.0)
