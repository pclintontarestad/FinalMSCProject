...
def H_tripod_t(t: float, args: dict) -> Qobj:
    """
    Effective dark-state polariton Hamiltonian (adiabatic elimination of |e>)."""
    ...
    H += gplus_t * (a_plus * S_plus.dag() + a_plus.dag() * S_plus)
    H += gminus_t * (a_minus * S_minus.dag() + a_minus.dag() * S_minus)
...
def run_read_phase(psi_in: Qobj, c_ops=None, h_args: dict | None = None):
    ...
    def g_plus_read(t: float) -> float:
        mirrored_t = T_READ_END - t
        base = g_pulsed(mirrored_t, g_plus)
        return -base if USE_READ_PHASE_FLIP else base

    def g_minus_read(t: float) -> float:
        mirrored_t = T_READ_END - t
        base = g_pulsed(mirrored_t, g_minus)
        return -base if USE_READ_PHASE_FLIP else base
