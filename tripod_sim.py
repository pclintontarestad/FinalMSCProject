...
def compute_full_cycle_metrics(
    ...
    # Track bright population using the instantaneous coupling-defined basis.
    U_write_end = dark_bright_unitary_from_couplings(Omega_c(tlist[-1]), g_minus, g_plus)
    U_read_start = dark_bright_unitary_from_couplings(Omega_c(tlist_read[0]), g_minus, g_plus)

    bright_proj_write = bright_projector_full(U_write_end)
    bright_proj_read = bright_projector_full(U_read_start)
...
def bright_population_trajectory(states, times, omega_c_func=Omega_c, g_minus_val=g_minus, g_plus_val=g_plus):
    ...
    for idx, (t, state) in enumerate(zip(times, states)):
        U_t = dark_bright_unitary_from_couplings(omega_c_func(t), g_minus_val, g_plus_val)
        bright_proj = bright_projector_full(U_t)
...
def su3_leakage_diagnostics(
    ...
    g_minus_use = g_minus if g_minus_val is None else g_minus_val
    g_plus_use = g_plus if g_plus_val is None else g_plus_val
    ...
    U_init = dark_bright_unitary_from_couplings(omega_c_func(tlist[0]), g_minus_use, g_plus_use)
    ...
    for idx, (t, state) in enumerate(zip(tlist, res.states)):
        U_t = dark_bright_unitary_from_couplings(omega_c_func(t), g_minus_use, g_plus_use)
        bright_proj = bright_projector_full(U_t)
        bright_pop[idx] = expect(bright_proj, state)
