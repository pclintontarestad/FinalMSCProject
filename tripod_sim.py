...
def H_tripod_t(t: float, args: dict) -> Qobj:
    ...
    gminus_t = g_pulsed(t, gminus_val) if not callable(g_minus_runtime) else gminus_val
    H += gminus_t * (a_minus * sig_egm1.dag() + a_minus.dag() * sig_egm1)

    # σ+ coupling: |g_{+1};1_+> ↔ |e;0_+> (Δm = +1)
    gplus_val = evaluate_coupling(g_plus_runtime, t)
    gplus_t = g_pulsed(t, gplus_val) if not callable(g_plus_runtime) else gplus_val
    ...
    for idx, (t, state) in enumerate(zip(tlist, res.states)):
        ...
        if rho_g is not None and U_ms is not None:
            ...
            if dark_pop > np.sqrt(EPS):
                ...
                logical_purity[idx] = logical_norm / dark_pop
