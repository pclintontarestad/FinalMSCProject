...
Delta_phys = 2 * np.pi * 100e6  # moderate one-photon detuning for stable EIT

# Control/probe Rabi frequencies (Hz)
Omega_C_phys = 2 * np.pi * 3e6  # control Rabi frequency
Omega_P_phys = 2 * np.pi * 0.3e6  # probe scale (used to set coupling g)
...
P_ground_full = P_gm1_full + P_g0_full + P_gp1_full
...
def run_read_phase(psi_in: Qobj, c_ops=None, h_args: dict | None = None):
    """Propagate through the read window with time-reversed, sign-flipped couplings."""
...
def plot_write_read_comparison(alpha_plus: complex = 1.0, alpha_minus: complex = 0.0):
    """Compare photon dynamics during write and the time-reversed read."""

    res_write = run_trajectory(alpha_plus, alpha_minus)
    _, _, _, res_read = run_three_phase_sequence(alpha_plus, alpha_minus)

    n_write = expect(N_plus_full + N_minus_full, res_write.states)
    n_read = expect(N_plus_full + N_minus_full, res_read.states)[::-1]

    plt.figure(figsize=(8, 5))
    plt.plot(tlist / Gamma_e * 1e6, n_write, label="Write (absorption)")
    plt.plot(tlist / Gamma_e * 1e6, n_read, label="Read (time-reversed)")
    plt.xlabel("Time (µs)")
    plt.ylabel(r"$\langle n_+ + n_- \rangle$")
    plt.title("Time-reversal check of photon dynamics")
    plt.legend()
    plt.grid(True, alpha=0.3)
    save_current_figure("write_read_time_reversal.pdf")


def check_storage_darkness(E_phys: np.ndarray, alpha_plus: complex = 1.0, alpha_minus: complex = 0.0) -> float:
    """Return overlap of the stored state with the encoder dark mode."""
    ...
    ground_amp = np.array(
        [
            ket_gm1_vacuum.overlap(final_state),
            ket_g0_vacuum.overlap(final_state),
            ket_gp1_vacuum.overlap(final_state),
        ]
    )

    dark_vec = E_phys[:, 0]
    overlap = np.abs(dark_vec.conj().T @ ground_amp) ** 2
...
def dark_bright_unitary_from_couplings(
    ...
    # Stabilise the basis to the nearest unitary to suppress drift
    u_q, _, vh_q = np.linalg.svd(Q)
    Q = u_q @ vh_q
...
def project_ground_density(rho_full: Qobj) -> Qobj:
    """Project a full density matrix onto the ground manifold and renormalise."""

    rho_g = P_ground_full * rho_full * P_ground_full
    trace_val = rho_g.tr()
    if abs(trace_val) < np.sqrt(EPS):
        return rho_g
    return rho_g / trace_val
...
def su3_leakage_diagnostics(
    ...
    r_leak_raw = np.zeros(n_steps)
    for idx in range(n_steps):
        S = np.array([exps[j][idx] / 2 for j in range(8)])
        r_leak_raw[idx] = 4.0 / 3.0 - np.sum(np.abs(S) ** 2)
    r_leak = r_leak_raw
...
if __name__ == "__main__":
    try:
        ...
        overlap_dark = check_storage_darkness(E_phys)
        run_outputs.append(f"Stored state overlap with dark mode: {overlap_dark:.6f}")
