...
def bundle_figures_pdf(
    output_path: Path = FIGURES_DIR / "tripod_figures_bundle.pdf",
    figures: list[Path] | None = None,
) -> Path | None:
    """Append all generated figure PDFs into a single bundle.

    If ``figures`` is provided, only those paths are bundled. Otherwise, all
    ``*.pdf`` files in ``thesis_figures`` (sorted alphabetically) are merged.
    Returns the bundle path if at least one page is written; otherwise ``None``.
    """

    _ensure_figures_dir()

    pdf_paths = figures if figures is not None else sorted(FIGURES_DIR.glob("*.pdf"))
    ...
    for pdf in pdf_paths:
        try:
            ...
            logging.warning("Skipping %s during bundle creation: %s", pdf, exc)
            continue
    ...
    if total_pages == 0:
        logging.info("No PDF figures found to bundle; skipping bundle creation.")
        return None
    ...
    logging.info(
        "Bundled %d pages from %d PDFs into %s", total_pages, len(pdf_paths), output_path
    )
    return output_path
...
if __name__ == "__main__":
    try:
        ...
        bundle_path = bundle_figures_pdf()
        if bundle_path is not None:
            run_outputs.append(f"Combined figure bundle: {bundle_path}")
