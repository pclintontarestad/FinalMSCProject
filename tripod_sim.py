...
def H_tripod_t(t: float, args: dict) -> Qobj:
    ...
    gminus_t = g_pulsed(t, g_minus_runtime)
    H += gminus_t * (a_minus * sig_egm1.dag() + a_minus.dag() * sig_egm1)

    # σ+ coupling: |g_{+1};1_+> ↔ |e;0_+> (Δm = +1)
    gplus_t = g_pulsed(t, g_plus_runtime)
...
def compute_full_cycle_metrics(
    ...
    bright_proj_write = bright_projector_full(tlist[-1], Omega_c, g_minus, g_plus)
    bright_proj_read = bright_projector_full(tlist_read[0], Omega_c_read, g_minus, g_plus)
...
def bright_projector_full(
    t: float,
    omega_c_func=Omega_c,
    g_minus_val: float = g_minus,
    g_plus_val: float = g_plus,
) -> Qobj:
    """
    ...
    Oc = omega_c_func(t)
    gminus_t = g_pulsed(t, g_minus_val)
    gplus_t = g_pulsed(t, g_plus_val)
