...
2. Run the module directly to reproduce the exploratory plots and console
   ...
   - Encoder trajectory plots (coefficient magnitudes, overlaps, and Gram
     eigenvalues vs time)
   - SU(3) leakage dashboard (bright population, leakage, logical purity) and
     transparency-window scan
   - Read-window photon re-emission and a small retrieval-fidelity landscape

   All individual figures are bundled into
   `thesis_figures/tripod_figures_bundle.pdf` for quick review. A concise
   summary of the run (storage efficiency, leakage metrics, tomography
   fidelities, etc.) is also written to `tripod_run_outputs.txt` in the
   repository root for later inspection.
